...
//  Rentory
//
//  Owns the user-facing iCloud sync state and the round trips between
//  the local backup package and the CloudKit private database. Conflict
//  resolution is intentionally simple: compare the remote modification
//  date against the device's last successful sync; on a tie or remote-
//  is-newer we replace local, otherwise we upload local. An empty
//  device always imports remote when present so a fresh install picks
//  up the user's records.
//
//  Split out of the original iCloudSyncStatusService.swift so the
//  lightweight availability struct can live next to a similarly-sized
//  file rather than next to a 350-line orchestrator.
//
